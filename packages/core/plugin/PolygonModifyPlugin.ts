...
const actionHandler: fabric.Control['actionHandler'] = function (eventData: MouseEvent, transform: fabric.Transform, x: number, y: number) {
...
const polygonPositionHandler = function (this: PointIndexControl, dim: any, finalMatrix: any, fabricObject: any) {
...
class PolygonModifyPlugin implements IPluginTempl {
  ...
  activeEdit() {
    ...
    if (poly && poly.type === EditorTypeEnum.Polygon) {
      this._ensureEvent(poly);
      if (poly.points == null) return;
      const lastControl = poly.points.length - 1;
      const This = this;
      poly.controls = poly.points.reduce<Record<string, PointIndexControl>>(function (acc, point, index) {
