...
class GroupTextEditorPlugin implements IPluginTempl {
  ...
  _bedingTextEditingEvent(textObject: fabric.IText, groupObj: fabric.Group) {
    ...
    tempText.on('editing:exited', () => {
      ...
      // 进入编辑模式时触发
      textObject.set({
        ...pick(attrs, ['fill', 'fontSize', 'fontStyle', 'fontFamily', 'lineHeight', 'backgroundColor']),
        text: tempText.text,
        visible: true
      });
      groupObj.addWithUpdate();
  ...
  isText(obj: fabric.Object) {
    const type = obj.type as unknown as EditorTypeEnum; // 类型断言
    return type && [EditorTypeEnum.IText, EditorTypeEnum.Text, EditorTypeEnum.TextBox].includes(type);
  }
