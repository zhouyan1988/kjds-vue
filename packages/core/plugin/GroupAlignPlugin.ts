...
type IPlugin = Pick<GroupAlignPlugin, 'left' | 'right' | 'xcenter' | 'ycenter' | 'top' | 'bottom' | 'xequation' | 'yequation'>;
...
class GroupAlignPlugin implements IPluginTempl {
  ...
  xequation() {
    ...
    if (activeObject && activeObject.type === EditorTypeEnum.ActiveSelection) {
  ...
  yequation() {
    const { canvas } = this;
    const activeObject: any = (canvas.getActiveObject() as fabric.ActiveSelection) || {
      top: 0,
      height: 0
    };
    // width属性不准确，需要坐标换算
