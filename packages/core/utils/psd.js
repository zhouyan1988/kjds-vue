    ...
    return childrensFilter.map(async (item) => {
      ...
      const typeList = [
        ...
        },
        {
          ...
          attr: ['text'],
          async customTransform(item) {
            const { FontSize, FillColor, Tracking } = item.textProperties.EngineDict.StyleRun.RunArray[0].StyleSheet.StyleSheetData;
            return {
      ...
      if (transformTypeInfo) {
        ...
        const customTransformAttr = transformTypeInfo.customTransform ? await transformTypeInfo.customTransform(item) : null;
