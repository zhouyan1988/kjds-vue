...
import { EditorTypeEnum, EXTRA_ATTRIBUTES, FabricObjectVO } from '@/utils/editor';
...
function transformText(objects: any) {
  ...
  objects.forEach((item: any) => {
    if (item.objects) {
      ...
      item.type === EditorTypeEnum.Text && (item.type = EditorTypeEnum.TextBox);
    }
  });
}
...
class ServersPlugin implements IPluginTempl {
  ...
  private _emitSelectEvent() {
    ...
    // @ts-ignore
    const actives = this.canvas.getActiveObjects().filter((item) => !(item instanceof fabric.GuideLine)); // 过滤掉辅助线
  ...
  renderITextPath(textPaths: Record<'id' | 'path', any>[]) {
    textPaths.forEach((item) => {
      const object: FabricObjectVO = this.canvas.getObjects().find((o: FabricObjectVO): boolean => o.id === item.id);
      if (object) {
  ...
  // 加载 JSON 之后，完成改回选项
  changeOptionsObject(opts: Record<'id', any>[]) {
    opts.forEach((item) => {
      const object: FabricObjectVO = this.canvas.getObjects().find((o: FabricObjectVO): boolean => o.id === item.id);
      if (object) {
        object.set('type', EditorTypeEnum.Options);
      }
    });
  }
  ...
  async loadJSON(jsonFile: string | object, callback?: () => void) {
    ...
    const opts: Record<'id', any>[] = [];
    temp.objects.forEach((item: any) => {
      !item.id && (item.id = uuid());
      if (item.type === EditorTypeEnum.Options) {
        item.type = EditorTypeEnum.Text;
        opts.push({ id: item.id });
      } else if (item.type === EditorTypeEnum.IText && item.path) {
  ...
  _getSaveSvgOption() {
    const workspace = this.canvas.getObjects().find((item: FabricObjectVO) => item.id === 'workspace');
    let fontFamilyArry = this.canvas
      .getObjects()
      .filter((item: FabricObjectVO): boolean => item.type == EditorTypeEnum.TextBox)
      // @ts-ignore
      .map((item: FabricObjectVO) => item.fontFamily);
  ...
  _getSaveOption() {
    const workspace: FabricObjectVO = this.canvas.getObjects().find((item: FabricObjectVO) => item.id === 'workspace');

    const { left, top, width, height } = workspace as FabricObjectVO;
    const option = {
      name: 'New Image',
      format: 'png',
      quality: 1,
  ...
  clear() {
    this.canvas.getObjects().forEach((obj: FabricObjectVO) => {
      if (obj.id !== 'workspace') {
