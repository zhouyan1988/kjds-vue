...
const innerUpClasses = ref(`${prefixCls}-handler-up-inner ${iconPrefixCls} ${iconPrefixCls}-ios-arrow-up`);
const innerDownClasses = ref(`${prefixCls}-handler-down-inner ${iconPrefixCls} ${iconPrefixCls}-ios-arrow-down`);
const upClasses = ref([
  `${prefixCls}-handler`,
...
const getStringValue = (number: number | undefined) => {
  ...
  const numString = mergedPrecision.value ? number.toFixed(mergedPrecision.value).replace(/\.?0+$/, '') : String(number);
...
const nextStep = (method: StepMethods, event: Event) => {
  if ((method === 'plus' && (isMax.value || props.upDisabled)) || (method === 'minus' && (isMin.value || props.downDisabled))) {
    return;
  }
...
<template>
  <div :class="wrapClasses">
    ...
    <div v-else-if="!hideButton" :class="handlerClasses">
      <a :class="upClasses" v-bind="clearRepeatTimerProps" @mousedown="plus">
        <span :class="innerUpClasses"></span>
      </a>
      <a :class="downClasses" v-bind="clearRepeatTimerProps" @mousedown="minus">
