...
<template>
  <div class="box">
    ...
    <!-- 纯色选择器 -->
    <ColorPicker v-show="!isGradient" v-model="fill" alpha @on-change="changePureColor" />
  </div>
</template>

<script setup name="ColorSelector" lang="ts">
...
const cssToFabricGradient = (val, activeObject) => {
  ...
  return generateFabricGradientFromColorStops(handlers, activeObject.width, activeObject.height, val.type, val.degree);
};
// 纯色颜色
