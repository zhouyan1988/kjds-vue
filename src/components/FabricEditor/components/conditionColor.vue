...
const getObjectAttr = (e) => {
  ...
  if (activeObject) {
    const con = activeObject.get('conditionColor');
    if (con) {
      baseAttr.conditionColor = activeObject.get('conditionColor');
    } else {
      baseAttr.conditionColor = {
        enable: false,
        option: ''
      };
      changeCommon('conditionColor', baseAttr.conditionColor);
    }
  }
};
...
<template>
  <div v-if="isOne && !isGroup && !isImage && !isOptions" class="box attr-item-box">
    <Divider plain orientation="left"><h4>颜色联动</h4></Divider>
    <div>
      <div class="hd-wrap">
        <div class="hd">
          <span>启用颜色联动</span>
        ...
        <iSwitch v-model="baseAttr.conditionColor.enable" size="large" class="switch" @on-change="onSwitchChange">
          <template #open>
            <span>开启</span>
          </template>
          <template #close>
      ...
      <template v-if="baseAttr.conditionColor.enable">
        <Form ref="formDynamicColor" :model="baseAttr.conditionColor" style="width: 277px" label-position="top">
          <FormItem label="关联选项">
            <Select v-model="baseAttr.conditionColor.option" @on-change="changeOption">
