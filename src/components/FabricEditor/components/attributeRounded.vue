        ...
        <Col :span="18" flex="1">
          <Form :label-width="40" class="form-wrap">
            <FormItem :label="$t('attributes.rx_ry')">
              <Slider v-model="baseAttr.roundValue" :max="300" @on-input="(value) => changeCommon(value)"></Slider>
            </FormItem>
          </Form>
        </Col>
        <Col :span="6" flex="1">
          <InputNumber v-model="baseAttr.roundValue" :min="0" :max="300" @on-change="(value) => changeCommon(value)"></InputNumber>
        </Col>
      </Row>
    </div>
...
const { canvasEditor, isOne, isMatchType } = useSelect([EditorTypeEnum.Rect]);
