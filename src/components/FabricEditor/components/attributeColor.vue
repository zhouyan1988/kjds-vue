  ...
  <div v-if="isOne && !isGroup && !isImage && !isOptions" class="box attr-item-box">
    ...
    <div class="bg-item">
      <Tooltip placement="top" theme="light">
        ...
        <template #content>
          <l-color-picker v-model:value="baseAttr.fill" :modes="['渐变', '纯色']" @change="colorChange" @native-pick="dropColor"></l-color-picker>
        </template>
      </Tooltip>
    </div>
...
const colorChange = (value) => {
  ...
  if (activeObject) {
    ...
    if (value.mode === '纯色') {
      ...
      const currentGradient = cssToFabricGradient(toRaw(value.stops), activeObject.width, activeObject.height, value.angle);
