...
<template>
  <div v-if="isOne && state.type === EditorTypeEnum.Image" class="box">
    ...
    <Collapse>
      ...
      <Panel name="2">
        ...
        <template #content>
          <!-- 有参数滤镜与组合参数滤镜 -->
          <div>
            <div v-for="item in [...state.paramsFilters, ...state.combinationFilters]" :key="item.type" class="filter-item has-params">
              <Checkbox v-model="item.status" @on-change="changeFiltersByParams(item.type)">
                {{ $t('filters.' + item.type) }}
              </Checkbox>
              <div v-if="item.status" class="content">
                <div v-for="info in item.params" :key="info" class="content slider-box">
                  <div v-if="info.uiType === uiType.SELECT">
                    <RadioGroup v-model="info.value" @on-change="changeFiltersByParams(item.type)">
                      <Radio v-for="listItem in info.list" :key="listItem" :label="listItem">
                  ...
                  <div v-if="info.uiType === uiType.COLOR">
                    <ColorPicker v-model="info.value" alpha size="small" @on-change="changeFiltersByParams(item.type)" />
                  </div>
                </div>
              </div>
            </div>
...
<script name="Filter" setup lang="ts">
...
const handleSelectOne = () => {
  ...
  if (activeObject) {
    ...
    if (state.type === EditorTypeEnum.Image) {
      // 无参数滤镜回显
...
onMounted(() => {
  canvasEditor.on(SelectEvent.ONE, handleSelectOne);
});

onBeforeUnmount(() => {
  canvasEditor.off(SelectEvent.ONE, handleSelectOne);
});
