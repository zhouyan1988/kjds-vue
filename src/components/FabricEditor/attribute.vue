  ...
  <div v-if="isOne" class="box">
    ...
    <div v-show="isMatchType">
      ...
      <Row v-if="selectType === 'polygon'" :gutter="12">
        <Col flex="0.5">
          <InputNumber v-model="baseAttr.points.length" :min="3" :max="30" append="边数" @on-change="changeEdge"></InputNumber>
        </Col>
      </Row>
      <!-- 颜色 -->
...
import colorSelector from '@/components/FabricEditor/colorSelector.vue';
...
const changeEdge = (value) => {
  ...
  activeObjects[0].set('points', getPolygonVertices(value, Math.min(activeObjects[0].width, activeObjects[0].height) / 2));
