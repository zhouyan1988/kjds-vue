...
<template>
  <div>
    ...
    <div class="search-box">
      <Select v-model="typeValue" class="select" :disabled="pageLoading" @on-change="changeSelectType">
        <Option v-for="item in typeList" :key="item.value" :value="item.value">
          {{ item.label }}
        </Option>
      </Select>
      <Input v-model="searchKeyWord" class="input" :placeholder="`在${typeText}中搜索`" search :disabled="pageLoading" @on-search="startGetList" />
    </div>
    <!-- 列表 -->
    <div id="myTemplBox" style="height: calc(100vh - 108px)">
      <Scroll v-if="showScroll" key="mysscroll" :on-reach-bottom="nextPage" :height="scrollHeight" :distance-to-edge="[-1, -1]">
        <!-- 列表 -->
        <div class="list-box">
          <Tooltip v-for="info in pageData" :key="info.src" :content="info.name" placement="top">
            <div class="tmpl-img-box">
              <Image lazy :src="info.previewSrc" fit="contain" height="100%" :alt="info.name" @click="beforeClearTip(info)" />
            </div>
          </Tooltip>
        </div>
        <Spin size="large" fix :show="pageLoading"></Spin>

        <Divider v-if="isDownBottm" plain>已经到底了</Divider>
      </Scroll>
    </div>
  </div>
</template>
...
const getTempData = async (info) => {
  ...
  if (route.query.admin) {
    router.push('/artworks/details?tempId=' + info.id + '&admin=true');
  } else {
    router.push('/artworks/details?tempId=' + info.id);
  }
