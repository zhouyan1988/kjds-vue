...
<template>
  <div>
    ...
    <div class="search-box">
      <Dropdown placement="bottom-start" style="margin-right: 10px" transfer @on-click="createType">
      ...
      <Input v-model="filters.name.$contains" class="input" placeholder="请输入关键词" search :disabled="pageLoading" @on-search="startGetList" />
    </div>

    <!-- 面包屑导航 -->
    <Breadcrumb>
      <BreadcrumbItem v-for="(item, i) in filePath" :key="item.id" @click="toFile(item.parentId, i)">
        {{ item.name }}
      </BreadcrumbItem>
    </Breadcrumb>

    <!-- 列表 -->
    <div id="myFileTemplBox" style="height: calc(100vh - 160px)">
      <Scroll v-if="showScroll" key="myFileTemplBox" :on-reach-bottom="nextPage" :height="scrollHeight" :distance-to-edge="[-1, -1]">
        <!-- 列表 -->
    ...
    <!-- 创建设计 -->
    <modalSzie ref="modalSizeRef" :title="$t('importFiles.createDesign.title')" @set="customSizeCreate"></modalSzie>
  </div>
</template>
...
// 通用分页
const { pageData, showScroll, scrollHeight, isDownBottom, pageLoading, startPage, startGetList, nextPage } = usePageList({
  el: '#myFileTemplBox',
