...
<template>
  <div class="my-material">
    <Button icon="md-cloud-upload" long type="primary" @click="uploadImgHandule">
      {{ $t('myMaterial.uploadBtn') }}
    </Button>
    <div v-if="fileList.length" class="img-group">
      <Tooltip v-for="(info, i) in fileList" :key="`${i}-bai1-button`" :content="info.name" placement="top">
        <div class="tmpl-img-box">
          <Icon type="ios-trash" class="del-btn" color="red" @click="removeMaterialHandle(info.id)" />
          <Image lazy :src="info.imgUrl" fit="contain" height="100%" :alt="info.name" @click="addImgByElement" />
        </div>
      </Tooltip>
    </div>
    <div v-else class="tip">暂无素材</div>
  </div>
</template>

<script setup name="ImportTmpl" lang="ts">
const APPAPIHOST = import.meta.env.VITE_APP_APIHOST;
    ...
    .then((res) => {
      fileList.value = res.data.data.map((item) => {
        return {
          ...
          imgUrl: APPAPIHOST + item.attributes.img.data.attributes.url
        };
      });
      isLogin.value = true;
    })
