...
function getPageParams(page, typeValue, searchKeyWord, searchTypeKey, searchWordKey, pageSize, fields) {
  const query = {
    populate: {
      img: '*'
    },
...
export default function usePageList({ typeUrl, listUrl, searchTypeKey, searchWordKey, scrollElement, pageSize, fields = [] }) {
  ...
  const getPageData = async () => {
    pageLoading.value = true;
    try {
      const params = getPageParams(page.value, typeValue.value, searchKeyWord.value, searchTypeKey, searchWordKey, pageSize, fields);
