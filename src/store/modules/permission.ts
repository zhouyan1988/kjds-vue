...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const generateConstantRoutes = async (): Promise<RouteRecordRaw[]> => {
    const data = constantRoutes;
    const sdata = JSON.parse(JSON.stringify(data));
    // const rdata = JSON.parse(JSON.stringify(data));
    // const defaultData = JSON.parse(JSON.stringify(data));
    const sidebarRoutes = filterAsyncRouter(sdata);
    // const rewriteRoutes = filterAsyncRouter(constantRoutes, undefined, true);
    // const defaultRoutes = filterAsyncRouter(defaultData);
    // const asyncRoutes = filterDynamicRoutes(dynamicRoutes);
    // asyncRoutes.forEach((route) => {
    //   router.addRoute(route);
    ...
    // setSidebarRouters(constantRoutes.concat(sidebarRoutes));
    setSidebarRouters(sidebarRoutes);
    setDefaultRoutes(constantRoutes);
    // setTopbarRoutes(constantRoutes);
    return new Promise<RouteRecordRaw[]>((resolve) => resolve(data));
  };
