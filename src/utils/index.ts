...
import { AttribugesGroups, CategoryProductDataVO, GroupsChild, TheAttributesChild, TheChild } from '@/api/shop/types';
import { ref } from 'vue';
import { EditorTypeEnum, FabricObjectVO, OptArresVO } from '@/utils/editor';
 ...
 * 判断值是否存在于数组中
 */
export const isInArray = (arr: any[], value: any, field?: string) => {
...
export const handleAttribugesGroups = (data: AttribugesGroups): AttribugesGroups => {
  for (const groupsKey in data?.groups) {
    const group = data?.groups[groupsKey];
    if (group.attributes) {
      for (const attributesKey in group.attributes) {
        const attribute = group.attributes[attributesKey];
        if (String(group.default) == attributesKey) {
 ...
 * 根据规格选中状态，重新计算价格
 */
export const handlePrice = (product: CategoryProductDataVO, attr: AttribugesGroups, currency: string = '1', templJsonObjects?: FabricObjectVO[]) => {
  const selectedAttr: string[] = [];
  for (const groupsKey in attr?.groups) {
    const group: GroupsChild = attr?.groups[groupsKey];
    if (group.attributes) {
      for (const attributesKey in group.attributes) {
        const attribute = group.attributes[attributesKey];
        if (attribute.selected) {
          selectedAttr.push(attributesKey);
        }
      }
    }
  }
    ...
    for (let i = 0; i < templJsonObjects.length; i++) {
      ...
      if (curtTempl.type === EditorTypeEnum.Options) {
        ...
        if (curtOption && curtOption.setPrice && curtOption.price) {
          totalPrice = numericalArithmetic(totalPrice, curtOption.price, '+');
        }
      }
    }
  }
...
export const clearSelection = (attrs: TheAttributesChild) => {
  for (const attrsKey in attrs) {
    if (attrs.hasOwnProperty(attrsKey)) {
      attrs[attrsKey].selected = false;
    }
  }
};
